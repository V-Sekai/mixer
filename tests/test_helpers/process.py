...
class UvBpyProcess(Process):
    """
    Starts a uv bpy python process that runs a script using PyPI bpy package
    """

    def __init__(self):
        super().__init__()
        self._uv_path = "uv"
        logger.info("Using uv bpy process")

    def start(self, args: Optional[Iterable[Any]] = ()) -> str:
        popen_args = [self._uv_path, "run", "python"]
        popen_args.extend([str(arg) for arg in args])
...
class UvBpyServer(UvBpyProcess):
    """
    Starts a uv bpy process that runs a python server. The uv bpy can be controlled
    by sending python source code, similar to BlenderServer but using PyPI bpy.
    """
    ...
    def start(self, blender_args: List = None, env_override: Optional[Mapping[str, str]] = None):
        args = [self._path, f"--port={self._port}"]
        if self._ptvsd_port is not None:
            args.append(f"--ptvsd={self._ptvsd_port}")
        if self._wait_for_debugger:
            args.append("--wait_for_debugger")
    ...
    def connect(self):
        ...
        if not connected:
            message = (
                f"Cannot connect to uv bpy at 127.0.0.1:{self._port} after {int(time.monotonic() - start)} seconds.\n"
                + f"Command line was: {self.command_line}"
            )
    ...
    def send_string(self, script: str):
        # ensure that the scripts are processed one by one,
        # otherwise they get buffered here on startup and get all the scripts at once before
        # the initial synchronization is done
        buffer = script.encode("utf-8")
        length_buffer = encode_int(len(buffer))
        self._sock.send(length_buffer)
